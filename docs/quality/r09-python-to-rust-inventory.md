...
Iteration 5 replays the i32 control-flow product-backend E2E verifier natively in Rust while preserving the Python verifier unchanged:
...
This is another contract-level `TRANSFORM`: source variants, fixed inputs, both branch observables and artifact-causality checks remain equivalent while Python orchestration becomes idiomatic Rust process execution and assertions. Three independent backend E2E replays now show that Cargo reuses the already-built backend across integration binaries. Shared test support is therefore not required as a runtime optimization yet. The helper layer stays local through the checked-overflow replay so that its deliberately different failure/overflow observable can test the correct abstraction boundary before any common harness is extracted.

## Iteration 6 acceptance

Iteration 6 replays the checked-i32-overflow product semantic E2E verifier natively in Rust while preserving the Python verifier unchanged:
...
- the Rust replay deliberately does not require a specific CLR exception type or message, avoiding a stronger runtime-specific contract than the Python verifier protects;
- the existing Python verifier remains in backend convergence for differential evidence and is not deleted or weakened;
- the Rust replay is an ordinary integration test reached by `cargo test --workspace --tests` on both Ubuntu and Windows.

This is a contract-level `TRANSFORM`, and it establishes the abstraction boundary for the next refactor: shared Rust test support may own backend construction, rustc invocation, temporary workspaces and managed-consumer process mechanics, but each E2E contract must continue to own its observable/oracle. Local-flow, arithmetic and control-flow assert successful managed values and source-causal artifact changes; checked overflow instead asserts a required failure path. That difference should remain visible rather than be hidden behind an over-generalized test helper.
