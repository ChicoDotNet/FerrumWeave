...
2. Complete Rust E2E parity for managed-consumption families: static call, construction, instance call, property access and external assembly consumption.
3. Complete Rust E2E parity for multiple/mixed/heterogeneous export aggregation.
4. Reproduce the complete codegen-backend boundary contract natively in Rust.
5. Decide the long-term evidence role for historical R02/R03 and the managed-static-call upstream oracle; preserve them until that decision is explicit.
...
No `PYTHON_ONLY` or `RUST_PARTIAL` verifier was deleted.
