...
//! Minimal managed export emission used by the first FerrumWeave-owned MIR lowering slice.
//!
//! This module is intentionally value-driven: the emitted `Answer()` body receives its
//! `i32` from the compiler lowering layer. No business/test value is embedded here.
...
/// Emit a minimal IL-only managed library exposing:
///
/// `public static int FerrumWeave.RustApi.Answer()`
///
/// The returned value is supplied by the caller. The MIR lowering layer is
...
fn build_metadata(method_rva: u32) -> Vec<u8> {
    ...
    push_u32(&mut tables, 0x0010_0001); // Public | BeforeFieldInit
    ...
    push_u16(&mut tables, 0); // IL + managed default implementation flags
    ...
    push_u16(&mut tables, 0);
    push_u32(&mut tables, 0);
    push_u16(&mut tables, 0); // no public key
    push_u16(&mut tables, assembly_name);
    push_u16(&mut tables, 0); // neutral culture

    // AssemblyRef (0x23): System.Runtime, .NET 10 framework assembly.
