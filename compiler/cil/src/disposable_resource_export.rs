...
const FIELD_TOKEN_RELEASED: u32 = 0x0400_0002;

#[must_use]
pub fn emit_disposable_resource_assembly(
    assembly_name: &str,
    namespace: &str,
    type_name: &str,
    seed: i32,
    ...
    let ctor_body = build_resource_constructor_body(seed);
    let ctor_offset = CLR_HEADER_SIZE;
    let ctor_rva = SECTION_RVA + to_u32(ctor_offset);

    let dispose_body = build_dispose_body(release_increment);
...
fn build_resource_constructor_body(seed: i32) -> Vec<u8> {
    let mut code = Vec::new();
    code.push(0x02); // ldarg.0
    code.push(0x28); // call instance void System.Object::.ctor()
    push_u32(&mut code, MEMBER_REF_TOKEN_OBJECT_CTOR);
    code.push(0x02); // ldarg.0
    emit_i32_constant(&mut code, seed);
    code.push(0x7D); // stfld int32 releaseCount
    push_u32(&mut code, FIELD_TOKEN_RELEASE_COUNT);
    code.push(0x02); // ldarg.0
    code.push(0x16); // ldc.i4.0
    code.push(0x7D); // stfld bool released
    push_u32(&mut code, FIELD_TOKEN_RELEASED);
    code.push(0x2A); // ret
    tiny_method(code)
}

fn build_dispose_body(release_increment: i32) -> Vec<u8> {
    let mut code = Vec::new();
    code.push(0x02); // ldarg.0
    code.push(0x7B); // ldfld bool released
    push_u32(&mut code, FIELD_TOKEN_RELEASED);
    code.push(0x2D); // brtrue.s -> ret
    let branch_offset_index = code.len();
    code.push(0);

    code.push(0x02); // ldarg.0
    code.push(0x02); // ldarg.0
    code.push(0x7B); // ldfld int32 releaseCount
    push_u32(&mut code, FIELD_TOKEN_RELEASE_COUNT);
    emit_i32_constant(&mut code, release_increment);
    code.push(0x58); // add
    code.push(0x7D); // stfld int32 releaseCount
    push_u32(&mut code, FIELD_TOKEN_RELEASE_COUNT);
    ...
    let ret_index = code.len();
    code.push(0x2A); // ret
    let delta = ret_index
        .checked_sub(branch_offset_index + 1)
        .expect("Dispose branch target follows branch");
    code[branch_offset_index] = u8::try_from(delta).expect("Dispose short branch fits u8");
    tiny_method(code)
}
...
fn tiny_method(code: Vec<u8>) -> Vec<u8> {
    assert!(code.len() <= 63, "tiny CIL method body must fit 63 bytes");
    let mut body = Vec::with_capacity(code.len() + 1);
    body.push((u8::try_from(code.len()).expect("tiny code size fits u8") << 2) | 0b10);
    body.extend_from_slice(&code);
    body
}

fn emit_i32_constant(code: &mut Vec<u8>, value: i32) {
    match value {
        -1 => code.push(0x15),
        0 => code.push(0x16),
        1 => code.push(0x17),
...
fn build_metadata(
    ...
    let resource_name = push_string(&mut strings, type_name);
    let resource_namespace = push_string(&mut strings, namespace);
    let release_count_field_name = push_string(&mut strings, "releaseCount");
    let released_field_name = push_string(&mut strings, "released");
    let ctor_name = push_string(&mut strings, ".ctor");
    let dispose_name = push_string(&mut strings, "Dispose");
    let release_count_name = push_string(&mut strings, "ReleaseCount");
    ...
    let guid = vec![
        0x46, 0x57, 0x44, 0x49, 0x53, 0x50, 0x4F, 0x53, 0x41, 0x42, 0x4C, 0x45, 0x52, 0x45, 0x53,
        0x31,
    ];

    let mut blobs = vec![0_u8];
    let field_i32_signature = push_blob(&mut blobs, &[0x06, 0x08]);
    let field_bool_signature = push_blob(&mut blobs, &[0x06, 0x02]);
    ...
    for count in [1_u32, 2, 2, 2, 3, 1, 1, 1, 1] {
    ...
    for type_ref_name in [object_name, idisposable_name] {
        push_u16(&mut tables, 6);
        push_u16(&mut tables, type_ref_name);
    ...
    push_u16(&mut tables, resource_namespace);
    ...
    push_u16(&mut tables, 0x0001);
    push_u16(&mut tables, released_field_name);
    push_u16(&mut tables, field_bool_signature);
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("disposable resource emitter only needs compact metadata values"),
    }
}
...
fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("metadata value fits u32")
}
...
mod tests {
    ...
    fn disposable_resource_image_is_source_parameterized_and_deterministic() {
        ...
        assert_ne!(
            first,
            emit_disposable_resource_assembly("RustLibrary", "FerrumWeave", "RustResource", 5, 2)
        );
        for expected in ["RustResource", "Dispose", "ReleaseCount", "IDisposable"] {
            assert!(
                first
