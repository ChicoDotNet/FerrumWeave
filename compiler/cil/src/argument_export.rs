...
//! Managed export emission for FerrumWeave MIR argument/local data flow.
//!
//! The compiler lowering layer selects which Rust argument reaches the return
//! place. This emitter only translates that lowered argument index into the
//! corresponding managed method signature and `ldarg` instruction.
...
/// Emit `public static int Answer(int left, int right)` whose result is one of
...
fn build_method_body(argument_index: u8) -> [u8; 3] {
    // Tiny header for two bytes of IL, followed by ldarg.0/ldarg.1 and ret.
...
fn build_metadata(method_rva: u32) -> Vec<u8> {
    ...
    let guid = vec![
        0x46, 0x57, 0x41, 0x52, 0x47, 0x45, 0x58, 0x50, 0x4F, 0x52, 0x54, 0x30, 0x30, 0x30, 0x30,
        0x31,
    ];
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("argument export only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("argument export metadata index fits u16")
}

fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("argument export image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn argument_export_is_selection_sensitive() {
        let left = emit_i32_argument_export_assembly(0);
        let right = emit_i32_argument_export_assembly(1);
