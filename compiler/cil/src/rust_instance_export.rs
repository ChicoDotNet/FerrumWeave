...
/// Emits a minimal managed library that projects a Rust ADT as a constructible
/// CLR reference type with one public instance method returning an i32 value.
///
/// Names and behavior are inputs owned by the rustc-facing lowering boundary;
/// this emitter contains no milestone-specific type names or expected values.
#[must_use]
pub fn emit_constructible_i32_instance_assembly(
    assembly_name: &str,
    namespace: &str,
    type_name: &str,
    method_name: &str,
    ...
    let method_body = build_i32_method_body(value);
    let method_offset = align_usize(ctor_offset + ctor_body.len(), 4);
...
fn build_i32_method_body(value: i32) -> Vec<u8> {
...
fn build_metadata(
    ...
    let method_signature = push_blob(&mut blobs, &[0x20, 0x00, 0x08]);
    ...
    for count in [1_u32, 1, 2, 2, 1, 1, 1] {
...
mod tests {
    ...
    #[test]
    fn projected_instance_behavior_is_data_not_baked_output() {
        let first = emit_constructible_i32_instance_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustValue",
            "Answer",
        ...
        let second = emit_constructible_i32_instance_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustValue",
            "Answer",
        ...
        for marker in [
            ...
            b"RustValue".as_slice(),
            b"FerrumWeave".as_slice(),
            b"Answer".as_slice(),
        ] {
