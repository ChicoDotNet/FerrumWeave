...
//! Managed exception export emission for Rust failure projections.
//! The CLR surface and exception message are supplied by compiler lowering.
...
const MEMBER_REF_TOKEN_EXCEPTION_CTOR: u32 = 0x0A00_0001;
const USER_STRING_TOKEN_MESSAGE: u32 = 0x7000_0001;

/// Emits one public static `int32` method that throws
/// `System.InvalidOperationException(message)`.
#[must_use]
pub fn emit_i32_invalid_operation_export_assembly(
    assembly_name: &str,
    namespace: &str,
    type_name: &str,
    method_name: &str,
    ...
    let method_body = build_throw_body();
...
fn build_throw_body() -> Vec<u8> {
    const CODE_SIZE: u8 = 11;
    let mut body = Vec::with_capacity(usize::from(CODE_SIZE) + 1);
    body.push((CODE_SIZE << 2) | 0b10);
    body.push(0x72); // ldstr
    push_u32(&mut body, USER_STRING_TOKEN_MESSAGE);
    body.push(0x73); // newobj instance void System.InvalidOperationException::.ctor(string)
    push_u32(&mut body, MEMBER_REF_TOKEN_EXCEPTION_CTOR);
    body.push(0x7A); // throw
    body
}
...
fn build_metadata(
    ...
    let invalid_operation_name = push_string(&mut strings, "InvalidOperationException");
    ...
    let message_index = push_user_string(&mut user_strings, message);
    debug_assert_eq!(message_index, 1);
    pad_vec(&mut user_strings, 4);

    let guid = vec![
        0x46, 0x57, 0x45, 0x58, 0x43, 0x45, 0x50, 0x54, 0x49, 0x4F, 0x4E, 0x30, 0x30, 0x30, 0x30,
        0x31,
    ];

    let mut blobs = vec![0_u8];
    let method_signature = push_blob(&mut blobs, &[0x00, 0x00, 0x08]);
    let exception_ctor_signature = push_blob(&mut blobs, &[0x20, 0x01, 0x01, 0x0E]);
    ...
    // TypeRef row 2: [System.Runtime]System.InvalidOperationException.
    push_u16(&mut tables, 6);
    push_u16(&mut tables, invalid_operation_name);
    push_u16(&mut tables, system_namespace);
    ...
    // TypeDef row 2: public projected API type extending System.Object.
    ...
    // MethodDef row 1: public static int32 method(); the body always throws.
    ...
    // MemberRef row 1: instance void InvalidOperationException::.ctor(string).
    // MemberRefParent tag 1 is TypeRef; row 2 => (2 << 3) | 1 = 17.
    push_u16(&mut tables, 17);
    push_u16(&mut tables, ctor_name);
    push_u16(&mut tables, exception_ctor_signature);
    ...
    // AssemblyRef: System.Runtime 10.0.0.0.
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        0x80..=0x3FFF => {
            ...
            buffer.push(u8::try_from(value & 0xFF).expect("14-bit suffix fits u8"));
        }
        _ => panic!("exception export only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("exception export metadata index fits u16")
}
fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("exception export image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn exception_export_is_message_and_surface_sensitive() {
        let first = emit_i32_invalid_operation_export_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustApi",
            "ResultErrI32",
