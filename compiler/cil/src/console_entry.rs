...
//! Minimal managed console-entry emission for the R10 installed console slice.
//! The Rust-derived export remains independently consumable while Program.Main
//! becomes the real CLI entry point and delegates to that export MethodDef.
...
const METHOD_DEF_TOKEN_ANSWER: u32 = 0x0600_0001;
const METHOD_DEF_TOKEN_MAIN: u32 = 0x0600_0002;

/// Emits a managed executable assembly with both the ordinary FerrumWeave
/// `RustApi` export and a CLR `Program.Main` adapter. `Main` calls the export
/// MethodDef rather than duplicating its constant, preserving one causal value.
#[must_use]
pub fn emit_console_i32_entry_assembly(
    assembly_name: &str,
    root_namespace: &str,
    export_method_name: &str,
    value: i32,
) -> Vec<u8> {
    let answer_body = build_i32_return_body(value);
...
fn build_main_body() -> Vec<u8> {
    ...
    let mut body = Vec::with_capacity(usize::from(CODE_SIZE) + 1);
    body.push((CODE_SIZE << 2) | 0b10);
    body.push(0x28); // call int32 FerrumWeave.RustApi::<export>()
    push_u32(&mut body, METHOD_DEF_TOKEN_ANSWER);
    body.push(0x2A); // ret
    body
}
...
fn build_metadata(
    ...
    let rust_api_type_name = push_string(&mut strings, "RustApi");
    ...
    let export_method_name = push_string(&mut strings, export_method_name_value);
    let main_method_name = push_string(&mut strings, "Main");
    ...
    let i32_method_signature = push_blob(&mut blobs, &[0x00, 0x00, 0x08]);
    ...
    // TypeDef row 2: public FerrumWeave.RustApi, owning MethodDef row 1.
    push_u32(&mut tables, 0x0010_0001);
    push_u16(&mut tables, rust_api_type_name);
    push_u16(&mut tables, ferrumweave_namespace);
    push_u16(&mut tables, 5); // TypeDefOrRef-coded TypeRef row 1.
    push_u16(&mut tables, 1);
    push_u16(&mut tables, 1);

    // TypeDef row 3: public <RootNamespace>.Program, owning MethodDef row 2.
    push_u32(&mut tables, 0x0010_0001);
    ...
    // MethodDef row 1: public static int32 RustApi::<export>().
    push_u32(&mut tables, answer_rva);
    push_u16(&mut tables, 0);
    push_u16(&mut tables, 0x0096);
    push_u16(&mut tables, export_method_name);
    push_u16(&mut tables, i32_method_signature);
    push_u16(&mut tables, 1);

    // MethodDef row 2: public static int32 Program.Main().
    push_u32(&mut tables, main_rva);
    push_u16(&mut tables, 0);
    push_u16(&mut tables, 0x0096);
    push_u16(&mut tables, main_method_name);
    push_u16(&mut tables, i32_method_signature);
    push_u16(&mut tables, 1);
    ...
    // AssemblyRef row 1: System.Runtime, .NET 10 framework assembly.
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("console entry emission only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("console entry metadata index fits u16")
}

fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("console entry image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn console_entry_is_deterministic_value_sensitive_and_marks_main_as_cli_entry() {
        let first = emit_console_i32_entry_assembly("HelloFerrum", "HelloFerrum", "Answer", 137);
        assert_eq!(
            first,
            emit_console_i32_entry_assembly("HelloFerrum", "HelloFerrum", "Answer", 137)
        );
        assert_ne!(
            first,
