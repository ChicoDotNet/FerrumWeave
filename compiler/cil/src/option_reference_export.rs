...
const USER_STRING_TOKEN: u32 = 0x7000_0001;

/// Emits the smallest managed projection for Rust `Option<&str>` exports:
/// one Some-producing static method and one None-producing static method.
/// All names and the Some payload come from rustc-facing MIR lowering.
#[must_use]
pub fn emit_option_reference_export_assembly(
    assembly_name: &str,
    namespace: &str,
    type_name: &str,
    ...
    let some_body = build_some_method_body();
...
fn build_metadata(
    ...
    let user_string_index = push_user_string(&mut user_strings, some_value);
    debug_assert_eq!(user_string_index, 1);
    ...
    let string_method_signature = push_blob(&mut blobs, &[0x00, 0x00, 0x0E]);
    ...
    // TypeRef: [System.Runtime]System.Object. AssemblyRef row 1 => 6.
    ...
    for (rva, name) in [(some_rva, some_method_name), (none_rva, none_method_name)] {
        ...
        push_u16(&mut tables, string_method_signature);
...
fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("managed image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn option_reference_payload_is_data_driven() {
        let first = emit_option_reference_export_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustApi",
            "OptionSomeString",
        ...
        let second = emit_option_reference_export_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustApi",
            "OptionSomeString",
