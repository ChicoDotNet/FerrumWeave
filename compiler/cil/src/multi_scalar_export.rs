...
//! Crate-level scalar export emission.
//! Method identities and payloads are supplied by compiler lowering; this module
//! does not know source fixture names or expected values.
...
pub struct I32ConstantExport<'a> {
    pub method_name: &'a str,
    pub value: i32,
}

#[must_use]
pub fn emit_named_i32_constant_exports_assembly(
    assembly_name: &str,
    namespace: &str,
    type_name: &str,
    ...
    for export in exports {
        let body = build_i32_return_body(export.value);
...
fn build_metadata(
    ...
    // TypeDef: <Module> owns no methods because the next TypeDef also starts at row 1.
    push_u32(&mut tables, 0);
    ...
    // TypeDef: the Rust API surface owns every MethodDef row.
    ...
    for (method_rva, method_name) in method_rvas.iter().zip(method_names.iter()) {
        ...
        push_u16(&mut tables, 1); // no Param rows
    }
...
fn write_clr_header(header: &mut [u8], metadata_rva: u32, metadata_size: u32) {
    ...
    write_u32_at(header, 20, 0); // library: no entry point
}
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("multi scalar export only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("multi scalar export metadata index fits u16")
}
fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("multi scalar export image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn multiple_scalar_exports_are_identity_and_value_sensitive() {
        let first = emit_named_i32_constant_exports_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustApi",
            &[
        ...
        assert!(first.windows(10).any(|window| window == b"AlphaValue"));
        assert!(first.windows(9).any(|window| window == b"BetaValue"));
    }
}
