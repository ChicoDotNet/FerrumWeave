...
//! Managed export emission for FerrumWeave MIR control flow.
//!
//! The rustc-facing lowering layer proves which Rust MIR predicate and branch
//! results participate in the exported function. This module only translates
//! that lowered control-flow shape into verifiable CIL. It never inspects Rust
//! source and it does not know the certifier's business values.
...
pub fn emit_i32_control_flow_export_assembly(
    ...
    assert!(
        ...
        "control-flow true branch must select left or right"
    );
    assert!(
        (1..=2).contains(&false_argument),
        "control-flow false branch must select left or right"
    );
    assert_ne!(
...
fn build_metadata(method_rva: u32) -> Vec<u8> {
    ...
    let guid = vec![
        0x46, 0x57, 0x43, 0x54, 0x52, 0x4C, 0x46, 0x4C, 0x4F, 0x57, 0x30, 0x30, 0x30, 0x30, 0x30,
        0x31,
    ];
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("control-flow export only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("control-flow export metadata index fits u16")
}

fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("control-flow export image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn control_flow_export_is_predicate_sensitive_and_branches_at_runtime() {
        let equal = emit_i32_control_flow_export_assembly(I32ZeroPredicate::Equal, 1, 2);
