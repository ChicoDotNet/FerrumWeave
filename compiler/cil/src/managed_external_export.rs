...
//! External managed-call emission for FerrumWeave-owned MIR lowering.
//!
//! The compiler layer supplies the Rust-derived payload. This module emits the
//! narrow ECMA-335 reference to a separately compiled managed dependency; it
//! does not inspect Rust source, compile C#, or contain expected-result logic.
...
const MEMBER_REF_TOKEN_EXTERNAL_TRANSFORM: u32 = 0x0A00_0001;
...
fn build_method_body(payload: i32) -> Vec<u8> {
    ...
    body.push(0x28); // call int32 ExternalManaged.ExternalApi::Transform(int32)
    push_u32(&mut body, MEMBER_REF_TOKEN_EXTERNAL_TRANSFORM);
    body.push(0x2A);
    body
}
...
fn build_metadata(method_rva: u32) -> Vec<u8> {
    ...
    let external_type_name = push_string(&mut strings, EXTERNAL_TYPE_NAME);
    let external_namespace = push_string(&mut strings, EXTERNAL_NAMESPACE);
    ...
    let external_assembly_name = push_string(&mut strings, EXTERNAL_ASSEMBLY_NAME);
    pad_vec(&mut strings, 4);

    let guid = vec![
        0x46, 0x57, 0x45, 0x58, 0x54, 0x45, 0x52, 0x4E, 0x41, 0x4C, 0x43, 0x41, 0x4C, 0x4C, 0x30,
        0x31,
    ];
    ...
    for count in [1_u32, 2, 2, 1, 1, 1, 2] {
    ...
    // TypeRef 1: [System.Runtime]System.Object.
    push_u16(&mut tables, 6);
    push_u16(&mut tables, object_name);
    push_u16(&mut tables, system_namespace);

    // TypeRef 2: [External.Managed]ExternalManaged.ExternalApi.
    push_u16(&mut tables, 10);
    push_u16(&mut tables, external_type_name);
    push_u16(&mut tables, external_namespace);
    ...
    push_u16(&mut tables, external_method_name);
    push_u16(&mut tables, external_signature);

    // Assembly: FerrumWeave.Generated.
    ...
    // AssemblyRef 2: independently compiled External.Managed, default 1.0.0.0.
    push_u16(&mut tables, 1);
    push_u16(&mut tables, 0);
    push_u16(&mut tables, 0);
    push_u16(&mut tables, 0);
    push_u32(&mut tables, 0);
    push_u16(&mut tables, 0);
    push_u16(&mut tables, external_assembly_name);
    push_u16(&mut tables, 0);
    push_u16(&mut tables, 0);
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("external managed export only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("external managed export metadata index fits u16")
}

fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("external managed export image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn external_managed_export_is_payload_sensitive_and_names_dependency() {
        let payload_137 = emit_i32_export_with_external_managed_transform(137);
        let payload_211 = emit_i32_export_with_external_managed_transform(211);
        assert_ne!(payload_137, payload_211);
        for expected in [
            "External.Managed",
            "ExternalManaged",
            "ExternalApi",
