...
const MEMBER_REF_TOKEN_NULLABLE_I32_CTOR: u32 = 0x0A00_0001;
const TYPE_SPEC_TOKEN_NULLABLE_I32: u32 = 0x1B00_0001;

/// Emits the smallest managed projection for Rust `Option<i32>` exports:
/// two static methods returning `System.Nullable<int>`. Names and the Some
/// payload are supplied by rustc-facing MIR lowering.
#[must_use]
pub fn emit_option_value_export_assembly(
    assembly_name: &str,
    namespace: &str,
    type_name: &str,
    some_method_name: &str,
    none_method_name: &str,
    some_value: i32,
) -> Vec<u8> {
    let some_body = build_some_method_body(some_value);
    let some_offset = CLR_HEADER_SIZE;
    let some_rva = SECTION_RVA + to_u32(some_offset);

    let none_body = build_none_method_body();
    let none_offset = align_usize(some_offset + some_body.len(), 4);
    let none_rva = SECTION_RVA + to_u32(none_offset);

    let metadata_offset = align_usize(none_offset + none_body.len(), 4);
    ...
    section[some_offset..some_offset + some_body.len()].copy_from_slice(&some_body);
    section[none_offset..none_offset + none_body.len()].copy_from_slice(&none_body);
...
fn build_some_method_body(value: i32) -> Vec<u8> {
...
fn build_none_method_body() -> Vec<u8> {
    const CODE_SIZE: u8 = 7;
    let mut body = Vec::with_capacity(usize::from(CODE_SIZE) + 1);
    body.push((CODE_SIZE << 2) | 0b10);
    body.push(0x14); // ldnull
    body.push(0xA5); // unbox.any System.Nullable<int32>; null => default Nullable<int32>
...
fn build_metadata(
    some_rva: u32,
    none_rva: u32,
    assembly_name_value: &str,
    namespace_value: &str,
    type_name_value: &str,
    some_method_name_value: &str,
    none_method_name_value: &str,
) -> Vec<u8> {
    let mut strings = vec![0_u8];
    let module_name = push_string(&mut strings, &format!("{assembly_name_value}.dll"));
    ...
    let some_method_name = push_string(&mut strings, some_method_name_value);
    let none_method_name = push_string(&mut strings, none_method_name_value);
    let ctor_name = push_string(&mut strings, ".ctor");
    let assembly_name = push_string(&mut strings, assembly_name_value);
    let system_runtime_name = push_string(&mut strings, "System.Runtime");
    pad_vec(&mut strings, 4);

    let guid = vec![
        0x46, 0x57, 0x4F, 0x50, 0x54, 0x56, 0x41, 0x4C, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30,
        0x31,
    ];

    let mut blobs = vec![0_u8];
    // DEFAULT, 0 params, GENERICINST VALUETYPE TypeRef(Nullable`1), I4.
    let nullable_i32_method_signature =
        push_blob(&mut blobs, &[0x00, 0x00, 0x15, 0x11, 0x09, 0x01, 0x08]);
    // HASTHIS, 1 param, VOID, VAR 0. TypeSpec binds VAR 0 to int32.
    ...
    for count in [1_u32, 2, 2, 2, 1, 1, 1, 1] {
    ...
    // TypeRef rows: [System.Runtime]System.Object and System.Nullable`1.
    for name in [object_name, nullable_name] {
        push_u16(&mut tables, 6); // ResolutionScope AssemblyRef row 1.
    ...
    // TypeDef: public beforefieldinit projected API type extending System.Object.
    push_u32(&mut tables, 0x0010_0001);
    push_u16(&mut tables, type_name);
    push_u16(&mut tables, namespace);
    push_u16(&mut tables, 5); // TypeDefOrRef TypeRef row 1.
    push_u16(&mut tables, 1);
    push_u16(&mut tables, 1);
    ...
    for (rva, name) in [(some_rva, some_method_name), (none_rva, none_method_name)] {
        ...
        push_u16(&mut tables, name);
        push_u16(&mut tables, nullable_i32_method_signature);
        push_u16(&mut tables, 1); // Empty Param table.
    }

    // MemberRef: instance void System.Nullable<int32>::.ctor(!0), parent TypeSpec row 1.
    push_u16(&mut tables, 12); // MemberRefParent TypeSpec row 1.
    push_u16(&mut tables, ctor_name);
    push_u16(&mut tables, nullable_i32_ctor_signature);

    // TypeSpec: System.Nullable<int32>.
...
fn write_clr_header(header: &mut [u8], metadata_rva: u32, metadata_size: u32) {
    ...
    write_u32_at(header, 0x10, 0x0000_0001); // ILOnly.
    write_u32_at(header, 0x14, 0); // Library, no entry point.
}
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        0x80..=0x3FFF => {
            buffer.push(u8::try_from((value >> 8) | 0x80).expect("14-bit high byte fits u8"));
            buffer.push(u8::try_from(value & 0xFF).expect("14-bit low byte fits u8"));
        }
        0x4000..=0x1FFF_FFFF => {
            buffer.push(u8::try_from((value >> 24) | 0xC0).expect("29-bit byte fits u8"));
            buffer.push(u8::try_from((value >> 16) & 0xFF).expect("29-bit byte fits u8"));
            buffer.push(u8::try_from((value >> 8) & 0xFF).expect("29-bit byte fits u8"));
            buffer.push(u8::try_from(value & 0xFF).expect("29-bit byte fits u8"));
        }
        _ => panic!("compressed unsigned value exceeds ECMA-335 range"),
    }
}
...
mod tests {
    ...
    #[test]
    fn option_value_payload_is_data_driven() {
        let first = emit_option_value_export_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustApi",
            "OptionSomeI32",
        ...
        let second = emit_option_value_export_assembly(
            "RustLibrary",
            "FerrumWeave",
            "RustApi",
            "OptionSomeI32",
