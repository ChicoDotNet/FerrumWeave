...
//! Managed instance-call emission for FerrumWeave-owned MIR lowering.
//!
//! The compiler layer supplies the selected receiver type and payload. This
//! module translates that lowered operation into ECMA-335 metadata and IL: it
//! constructs the selected receiver, dispatches `ToString` through `callvirt`,
//! discards the string result, and returns the Rust-derived i32 payload.
...
const MEMBER_REF_TOKEN_TOSTRING: u32 = 0x0A00_0002;
...
pub enum ManagedInstanceReceiver {
    Object,
    StringBuilder,
}

impl ManagedInstanceReceiver {
...
#[must_use]
pub fn emit_i32_export_with_managed_instance_call(
    receiver: ManagedInstanceReceiver,
    payload: i32,
) -> Vec<u8> {
    emit_named_i32_export_with_managed_instance_call(receiver, payload, METHOD_NAME)
}

#[must_use]
pub fn emit_named_i32_export_with_managed_instance_call(
    receiver: ManagedInstanceReceiver,
    payload: i32,
    method_name: &str,
) -> Vec<u8> {
    ...
    let metadata = build_metadata(method_rva, receiver, method_name);
...
fn build_method_body(payload: i32) -> Vec<u8> {
    ...
    body.push(0x6F); // callvirt instance string <selected>::ToString()
    push_u32(&mut body, MEMBER_REF_TOKEN_TOSTRING);
    body.push(0x26); // pop returned string
...
fn build_metadata(
    ...
    let receiver_name = push_string(&mut strings, receiver.type_name());
    let receiver_namespace = push_string(&mut strings, receiver.namespace());
    ...
    let tostring_signature = push_blob(&mut blobs, &[0x20, 0x00, 0x0E]);
    ...
    for count in [1_u32, 2, 2, 1, 2, 1, 1] {
    ...
    // TypeRef 2: source-selected receiver.
    push_u16(&mut tables, 6);
    push_u16(&mut tables, receiver_name);
    push_u16(&mut tables, receiver_namespace);
    ...
    // public static int32 <caller-owned export>().
    ...
    push_u16(&mut tables, tostring_signature);
...
fn push_compressed_unsigned(buffer: &mut Vec<u8>, value: u32) {
    match value {
        ...
        _ => panic!("managed instance export only needs compact metadata values"),
    }
}
...
fn to_u16(value: usize) -> u16 {
    u16::try_from(value).expect("managed instance metadata index fits u16")
}
fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("managed instance image size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn instance_export_is_payload_and_receiver_sensitive() {
        let object_137 =
    ...
    #[test]
    fn named_instance_export_uses_caller_owned_method_name() {
        let image = emit_named_i32_export_with_managed_instance_call(
        ...
        assert!(image.windows(1).any(|window| window == [0x6F]));
    }
}
