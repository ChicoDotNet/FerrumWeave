...
//! Managed property emission for FerrumWeave-owned MIR lowering.
//!
//! The compiler layer supplies an i32 payload derived from Rust MIR. This
//! module translates that lowered operation into ECMA-335 metadata and IL for
//! `System.Text.StringBuilder.Length`: construct the receiver, call the setter,
//! call the getter, and return the managed getter observable.
...
const MEMBER_REF_TOKEN_SET_LENGTH: u32 = 0x0A00_0002;
const MEMBER_REF_TOKEN_GET_LENGTH: u32 = 0x0A00_0003;
...
#[must_use]
pub fn emit_i32_export_with_string_builder_length_property(payload: i32) -> Vec<u8> {
    emit_named_i32_export_with_string_builder_length_property(payload, METHOD_NAME)
}

#[must_use]
pub fn emit_named_i32_export_with_string_builder_length_property(
    payload: i32,
    method_name: &str,
) -> Vec<u8> {
...
fn build_method_body(payload: i32) -> Vec<u8> {
    const CODE_SIZE: u8 = 22;
    let mut body = Vec::with_capacity(usize::from(CODE_SIZE) + 1);
    body.push((CODE_SIZE << 2) | 0b10);
    body.push(0x73); // newobj instance void System.Text.StringBuilder::.ctor()
    push_u32(&mut body, MEMBER_REF_TOKEN_CTOR);
    body.push(0x25); // dup; retain receiver for getter after setter consumes one copy.
    body.push(0x20); // ldc.i4 payload
    body.extend_from_slice(&payload.to_le_bytes());
    body.push(0x6F); // callvirt instance void StringBuilder::set_Length(int32)
    push_u32(&mut body, MEMBER_REF_TOKEN_SET_LENGTH);
    body.push(0x6F); // callvirt instance int32 StringBuilder::get_Length()
    push_u32(&mut body, MEMBER_REF_TOKEN_GET_LENGTH);
    body.push(0x2A); // ret
    body
}
...
fn build_metadata(method_rva: u32, method_name: &str) -> Vec<u8> {
    ...
    let string_builder_name = push_string(&mut strings, "StringBuilder");
    let system_namespace = push_string(&mut strings, "System");
    let text_namespace = push_string(&mut strings, "System.Text");
    ...
    let set_length_name = push_string(&mut strings, "set_Length");
    let get_length_name = push_string(&mut strings, "get_Length");
    ...
    let set_length_signature = push_blob(&mut blobs, &[0x20, 0x01, 0x01, 0x08]);
    let get_length_signature = push_blob(&mut blobs, &[0x20, 0x00, 0x08]);
    ...
    for count in [1_u32, 2, 2, 1, 3, 1, 1] {
    ...
    // TypeRef 1: System.Object base type.
    push_u16(&mut tables, 6);
    push_u16(&mut tables, object_name);
    push_u16(&mut tables, system_namespace);

    // TypeRef 2: System.Text.StringBuilder property receiver.
    push_u16(&mut tables, 6);
    push_u16(&mut tables, string_builder_name);
    push_u16(&mut tables, text_namespace);
    ...
    // FerrumWeave.RustApi extends System.Object.
    ...
    // public static int32 <Rust-owned export>().
    ...
    // MemberRefs parent TypeRef row 2: (2 << 3) | TypeRef-tag(1) = 17.
    for (name, signature) in [
        (ctor_name, ctor_signature),
        (set_length_name, set_length_signature),
        (get_length_name, get_length_signature),
    ] {
...
fn to_u32(value: usize) -> u32 {
    u32::try_from(value).expect("artifact size fits u32")
}
...
mod tests {
    ...
    #[test]
    fn property_emission_uses_caller_owned_export_identity() {
        let image = emit_named_i32_export_with_string_builder_length_property(3, "ComputeResult");
        assert!(
            image
                .windows("ComputeResult".len())
                .any(|window| window == b"ComputeResult")
        );
        assert!(image.iter().filter(|&&byte| byte == 0x6F).count() >= 2);
    }
}
