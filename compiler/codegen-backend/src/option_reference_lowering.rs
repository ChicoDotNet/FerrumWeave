                ...
                for statement in &block.statements {
                    ...
                    match variant_name {
                        "Some" => {
                            ...
                            let value = lower_string_operand(tcx, mir, operand)?;
...
fn lower_string_operand<'tcx>(
    tcx: TyCtxt<'tcx>,
    mir: &Body<'tcx>,
    operand: &Operand<'tcx>,
) -> Result<String, String> {
    match operand {
        Operand::Constant(constant) => lower_string_constant(tcx, constant),
            ...
            for block in mir.basic_blocks.iter() {
                for statement in &block.statements {
                    ...
                    if source.replace(candidate).is_some() {
                        return Err(format!(
                            "Option Some payload local `{local:?}` has multiple MIR definitions"
                        ));
                    }
                }
            }
            let source = source.ok_or_else(|| {
                format!("Option Some payload local `{local:?}` has no MIR Rvalue::Use definition")
            })?;
            lower_string_operand(tcx, mir, source)
        }
        _ => Err(format!(
            "Option Some payload must resolve to a constant &str MIR operand, found {operand:?}"
        )),
    }
}

fn lower_string_constant<'tcx>(
    tcx: TyCtxt<'tcx>,
