...
pub(crate) struct LoweredResultFailure {
    pub(crate) method_name: String,
    pub(crate) value: i32,
}

/// Recognizes an exported `Result<i32, i32>::Err` directly from monomorphized MIR.
pub(crate) fn lower_result_failure(tcx: TyCtxt<'_>) -> Result<Option<LoweredResultFailure>, String> {
        ...
        for (item, _data) in cgu.items() {
            ...
            for block in mir.basic_blocks.iter() {
                for statement in &block.statements {
                    ...
                    if variant != "Err" { return Ok(None); }
                    if operands.len() != 1 {
                        return Err(format!("Result Err export `{symbol}` must carry exactly one MIR operand"));
                    }
                    let operand = operands.iter().next().expect("Result Err operand count checked");
                    let value = lower_i32_operand(tcx, mir, operand)?;
                    return Ok(Some(LoweredResultFailure {
    ...
    match operand {
        Operand::Constant(constant) => {
            let evaluated = constant.const_.eval(tcx, TypingEnv::fully_monomorphized(), constant.span)
                .map_err(|_| "could not evaluate Result Err constant from MIR".to_owned())?;
            let ConstValue::Scalar(scalar) = evaluated else {
                return Err("Result Err MIR constant is not scalar i32".to_owned());
            };
            scalar.to_i32().report_err().map_err(|_| "Result Err MIR scalar is not i32".to_owned())
        }
            ...
            for block in mir.basic_blocks.iter() {
                for statement in &block.statements {
                    ...
                    if source.replace(candidate).is_some() {
                        return Err(format!("Result Err payload local `{local:?}` has multiple MIR definitions"));
                    }
                }
            }
            let source = source.ok_or_else(|| format!("Result Err payload local `{local:?}` has no MIR Rvalue::Use definition"))?;
            lower_i32_operand(tcx, mir, source)
        }
        _ => Err(format!("Result Err payload must resolve to constant i32 MIR operand, found {operand:?}")),
    }
}
