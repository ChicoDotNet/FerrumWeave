...
use ferrumweave_cil::{
    ...
    emit_i32_invalid_operation_export_assembly, emit_managed_executable_with_i32_constant_exports,
    ...
    emit_named_i32_direct_call_export_assembly, emit_named_i32_export_with_external_managed_transform,
    emit_named_i32_export_with_managed_construction, emit_named_i32_export_with_managed_instance_call,
    emit_named_i32_export_with_string_builder_length_property, emit_named_i32_exports_assembly,
...
use crate_exports::{
    LoweredCrateI32Export, lower_heterogeneous_i32_exports, lower_multiple_constant_exports,
};
use disposable_resource_lowering::lower_disposable_resource;
use entry_point::lower_managed_entry_point;
...
impl CodegenBackend for FerrumWeaveCodegenBackend {
    ...
    fn codegen_crate<'a>(&self, tcx: TyCtxt<'_>) -> Box<dyn Any> {
        ...
        let entry_point = lower_managed_entry_point(tcx)
            .unwrap_or_else(|message| panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}"));

        let image = if entry_point.is_some() {
            emit_executable_crate(tcx, assembly_name)
        } else {
            emit_library_crate(tcx, assembly_name)
        };
    ...
    fn link(
        ...
        fs::copy(source, outputs.path(OutputType::Exe).as_path())
            .expect("FerrumWeave could not publish the managed assembly");
    }
}

fn emit_executable_crate(tcx: TyCtxt<'_>, assembly_name: &str) -> Vec<u8> {
    let mut export_storage = Vec::new();
    match lower_exported_i32(tcx) {
        Ok(LoweredI32Export::Constant(value)) => {
            let name = managed_export_method_name(tcx).unwrap_or_else(|message| {
                panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}")
            });
            export_storage.push((name, value));
        }
        Err(message)
            if message == "no externally visible monomorphized i32 export reached FerrumWeave codegen" => {}
        Ok(_) => panic!(
            "FERRUMWEAVE_MIR_LOWERING_FAILED: executable entry slice currently supports only optional constant i32 exports"
        ),
        Err(message) => panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}"),
    }

    let exports: Vec<I32ConstantExport<'_>> = export_storage
        .iter()
        .map(|(method_name, value)| I32ConstantExport {
            method_name,
            value: *value,
        })
        .collect();

    emit_managed_executable_with_i32_constant_exports(
        assembly_name,
        assembly_name,
        "FerrumWeave",
        "RustApi",
...
fn emit_library_crate(tcx: TyCtxt<'_>, assembly_name: &str) -> Vec<u8> {
    ...
    let option_value = if result_failure.is_none() && result_success.is_none() {
        lower_option_value_exports(tcx)
            .unwrap_or_else(|message| panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}"))
    } else {
        None
    };
    let option_reference = if result_failure.is_none()
        && result_success.is_none()
        && option_value.is_none()
    {
        lower_option_reference_exports(tcx)
            .unwrap_or_else(|message| panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}"))
    } else {
        None
    };
    ...
    if let Some(result) = result_failure {
        ...
        let heterogeneous = lower_heterogeneous_i32_exports(tcx)
            .unwrap_or_else(|message| panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}"));
        if let Some(exports) = heterogeneous {
            let exports: Vec<I32ExportBody<'_>> = exports
                .iter()
                .map(|export| match export {
                    LoweredCrateI32Export::Constant { method_name, value } => {
                        I32ExportBody::Constant {
                            method_name,
                            value: *value,
                        }
                    }
                    LoweredCrateI32Export::Argument { method_name, index } => {
                        I32ExportBody::Argument {
                            method_name,
                            index: *index,
                        }
                    }
                    LoweredCrateI32Export::Arithmetic {
                        method_name,
                        operation,
                    } => I32ExportBody::Arithmetic {
                        method_name,
                        operation: *operation,
                    },
                    LoweredCrateI32Export::ControlFlow {
                        ...
                        true_argument,
                        false_argument,
                    } => I32ExportBody::ControlFlow {
                        method_name,
                        predicate: *predicate,
                        true_argument: *true_argument,
                        false_argument: *false_argument,
                    },
                    LoweredCrateI32Export::DirectRustCall {
                        method_name,
                        operation,
                    } => I32ExportBody::DirectRustCall {
                        method_name,
                        operation: *operation,
                    },
                })
                .collect();
            emit_named_i32_exports_assembly(
                assembly_name,
                "FerrumWeave",
                "RustApi",
                &exports,
            ...
            let crate_exports = lower_multiple_constant_exports(tcx).unwrap_or_else(|message| {
                panic!("FERRUMWEAVE_MIR_LOWERING_FAILED: {message}")
            });
