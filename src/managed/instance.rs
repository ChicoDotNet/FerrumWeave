...
pub fn resolve_public_instance_member_ref(
    ...
    resolve_public_static_member_ref(image, namespace, type_name, method_name)
}

pub fn method_calls_instance_member_ref(
    image: &[u8],
    ...
    while cursor < body.len() {
        match body[cursor] {
            ...
            0x28 | 0x72 | 0x73 => cursor += 5,
            0x25 | 0x26 | 0x2A => cursor += 1,
            _ => {
                return Err(ManagedMetadataError::Unsupported(
                    "IL opcode is not projected by the R05 instance-call inspector",
                ));
            }
        }
    }
