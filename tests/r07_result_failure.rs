...
const EXPECTED_MESSAGE: &str = "FerrumWeave Result error";

#[test]
fn result_failure_crosses_managed_boundary_as_coherent_exception() {
    ...
    fs::copy(&assembly, consumer.join(PROBE_ASSEMBLY_FILE))
        .expect("place Rust-produced managed assembly beside R07 Result failure consumer project");
...
"#
    ...
    assert!(
        run.status.success(),
        "C# consumer must execute the Rust-defined Result failure projection:\n{}\n{}",
