...
const CONSUMER_ASSEMBLY_NAME: &str = "FerrumWeave.R07.ResultSuccessConsumer";

#[test]
fn result_success_crosses_managed_boundary_from_rust_source() {
    ...
    fs::create_dir_all(&source_dir).expect("create R07 Result success Rust source directory");
    fs::create_dir_all(&consumer).expect("create R07 Result success C# consumer directory");
    ...
    for value in [42, 73] {
        ...
        assert!(
            ...
            "R07 Result success source must build through .rsproj -> rustc -> FerrumWeave; a RED here must come from unsupported FerrumWeave lowering, not a legacy emitter:\nstdout:\n{}\nstderr:\n{}",
        ...
        if let Some(previous) = &previous_artifact {
            assert_ne!(
                previous, &bytes,
                "mutating only the Rust Ok payload must mutate the managed artifact",
            );
        }
        previous_artifact = Some(bytes);

        fs::copy(&assembly, consumer.join("RustLibrary.dll"))
            .expect("place Rust-produced assembly beside R07 Result success consumer project");
        let run = build_and_run_consumer(&consumer, "RustLibrary.dll");
        assert!(
            run.status.success(),
            "C# consumer must execute Rust-source causal Result success behavior:\n{}\n{}",
...
"#
    ...
    let build = Command::new("dotnet")
        ...
        .output()
        .expect("build R07 Result success C# consumer against Rust-produced managed assembly");
    assert!(
        build.status.success(),
        "C# must compile against the Rust-defined Result success projection:\n{}\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
    ...
    Command::new("dotnet")
        ...
        .current_dir(consumer)
        .output()
        .expect("execute R07 Result success C# consumer on CoreCLR")
}
...
fn unique_temp_dir() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "ferrumweave-r07-result-success-{}-{nonce}",
        std::process::id(),
    ))
}
