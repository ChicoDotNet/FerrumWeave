...
#[inline(never)]
fn tuples_and_structs() -> i32 {
    let tuple: (i32, i32) = (black_box(19), black_box(23));
    let (left, right) = tuple;
    let pair = AnswerPair { left, right };
    let AnswerPair { left, right } = pair;
    left + right
}
...
#[inline(never)]
fn shared_references() -> i32 {
    let pair = AnswerPair {
        left: black_box(19),
        right: black_box(23),
    };
    let first: &AnswerPair = &pair;
    let second: &AnswerPair = &pair;
