...
fn i32_arithmetic_is_source_causal_through_the_ferrumweave_backend() {
    ...
    fs::create_dir_all(&work).expect("temporary arithmetic directory should be created");
    ...
    fs::remove_dir_all(work).expect("temporary arithmetic directory should be removable");
}
...
fn compile_source(
    ...
    let source = work.join(format!("arithmetic_{label}.rs"));
    let artifact = work.join(format!("arithmetic_{label}.dll"));
    fs::write(&source, rust_source(operator, export_name))
        .expect("Rust arithmetic source should be written");
    ...
    assert!(
        artifact.is_file(),
        "managed artifact should be produced for arithmetic {label}"
    );
    artifact
}
...
fn unique_temp_directory() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "ferrumweave-i32-arithmetic-rust-{}-{nanos}",
        std::process::id()
    ))
}
